...
class OrderController extends Controller
{
    ...
    public function checkout()
    {
        ...
        $this->data['totalWeight']=$this->getTotalWeight();
	...
	public function doCheckout(Request $request)
	{
		...
		$order=\DB::transaction(function() use($params)
		{
			...
			$selectedShipping = $this->_getSelectedShipping($destination, $this->getTotalWeight(), $params['shipping_service']);
			...
			if ($order && $cartItems) {
				foreach ($cartItems as $item) {
					...
					// if ($orderItem) {
					// 	DataProduk::reduceStock($orderItem->product_id, $orderItem->qty);
					// }
